...
func AddLane(ctx context.Context, plannerID, title, description, color string, position int) (*PlannerLane, error) {
	log.Printf("AddLane: Received plannerID=%s, title=%s, position=%d", plannerID, title, position)
	...
	// Use an atomic operation to add the lane without reading first
	// This avoids timing issues with newly created planners
	...
	if err != nil {
		log.Printf("AddLane: Error adding lane to planner %s: %v", plannerID, err)
		return nil, err
	}
